...
public class PreTurnInteractor implements PreTurnInputDataBoundary {
    ...
    public void execute(PreTurnInputData inputData) {
        Player currentPlayer = preTurnDataAccessInterface.getPlayer(inputData.getCurrentPlayerIndex());
        ...
        for (FunctionalCard functionalCard : preTurnDataAccessInterface.getPlayerFunctionalCards()){
        ...
        if (game.getPlusN() > 0 && !hasPlusCard) //previousPlayer played a plusCard, and the currentPlayer doesnt have a plus card
            {
            numberCards.addAll(dataAccessInterface.drawNumberCards(inputData.getNumberCardsDeck(), game.getPlusN()));
        ...
        } else if (game.getPlusN() > 0 && hasPlusCard) {
